...
cat \
  "$ROOT_DIR/paths/data/datasets.yaml" \
  "$ROOT_DIR/paths/training/jobs.yaml" \
  "$ROOT_DIR/paths/inference/model-endpoints.yaml" \
  > "$ROOT_DIR/merged_output.yaml"

"$SWAGGER_CLI" bundle "$ROOT_DIR/tir.yaml" --outfile "$OUTPUT_DIR/openapi.yaml" --type yaml
